...
export function UserDetailPage() {
  ...
  const loadUser = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const [u, groups] = await Promise.all([fetchAdminUser(accessToken, idNum), fetchAdminGroups(accessToken)]);
      setUser(u);
      setAllGroups(groups);
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-6">
      <header className="flex flex-wrap items-center gap-3">
        ...
        <h1 className="font-heading text-2xl font-semibold tracking-tight md:text-3xl">{t('userDetailTitle')}</h1>
      </header>
          ...
          <Card className="border-border/80 shadow-sm">
              ...
              <div className="min-w-0 flex-1 space-y-1">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-2 pt-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium text-muted-foreground">{t('usersColStaff')}</span>
                    <Badge variant={user.is_staff ? 'default' : 'outline'} className="tabular-nums">
                      {user.is_staff ? t('usersStaffYes') : t('usersStaffNo')}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium text-muted-foreground">{t('usersColActive')}</span>
                    <Badge variant={isActiveDraft ? 'secondary' : 'muted'} className="tabular-nums">
            ...
            <CardContent className="space-y-6 border-t border-border/80 pt-6">
              <dl className="grid gap-3 text-sm sm:grid-cols-2">
                <div>
                  <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailUsername')}</dt>
                  <dd className="mt-0.5 font-mono">{user.username}</dd>
                </div>
                <div>
                  <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailId')}</dt>
                  <dd className="mt-0.5 tabular-nums">{user.id}</dd>
                </div>
                <div>
                  <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailFirstName')}</dt>
                  <dd className="mt-0.5">{user.first_name || '—'}</dd>
                </div>
                <div>
                  <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailLastName')}</dt>
                  <dd className="mt-0.5">{user.last_name || '—'}</dd>
                </div>
                <div className="sm:col-span-2">
                  <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailLastSeen')}</dt>
                  <dd className="mt-0.5">{formatDateTime(lastSeenFromUser(user) ?? undefined, locale)}</dd>
                </div>
              </dl>
              <div className="space-y-4 border-t border-border/80 pt-6">
                <div>
                  <h3 className="text-sm font-medium leading-none">{t('userDetailMembershipTitle')}</h3>
                  <p className="pt-1.5 text-xs text-muted-foreground">{t('userDetailMembershipHint')}</p>
                </div>
                {saveError ? (
                  <p className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive">
                    {saveError}
                ...
                <div className="flex flex-row items-start gap-3 rounded-md border p-3">
                  ...
                  <div className="space-y-1 leading-none">
                    <Label htmlFor="user-detail-active" className="cursor-pointer">
                      {t('usersColActive')}
                    </Label>
                    <p className="text-xs text-muted-foreground">{t('userDetailActiveEditableHint')}</p>
                  </div>
                </div>
                <div className="space-y-2 rounded-md border p-3">
                  <Label>{t('userDetailGroups')}</Label>
                  <p className="text-xs text-muted-foreground">{t('userDetailGroupsEditableHint')}</p>
                  {allGroups.length === 0 ? (
                      ...
                      {allGroups.map((g) => {
                        ...
                        return (
                          <li key={g.id} className="flex items-center gap-2">
                            ...
                            />
                            <label htmlFor={`user-detail-group-${g.id}`} className="cursor-pointer text-sm">
                              {g.name}
                            </label>
                          </li>
                        );
          ...
          <Card className="border-border/80 shadow-sm">
            ...
            <CardContent>
              {!preferences ? (
                ...
                <dl className="grid gap-3 text-sm sm:grid-cols-2">
                  <div>
                    <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailPrefTheme')}</dt>
                    <dd className="mt-0.5">{preferences.themeName ?? '—'}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailPrefLanguage')}</dt>
                    <dd className="mt-0.5">{preferences.language ?? '—'}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailPrefRegion')}</dt>
                    <dd className="mt-0.5">{preferences.region ?? '—'}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailPrefColorScheme')}</dt>
              ...
              <CardContent className="space-y-4">
                {!loginHistoryInView && events === null ? (
                  <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                    <p className="max-w-xl text-sm text-muted-foreground">{t('userDetailLoginHistoryDefer')}</p>
                    <Button type="button" variant="secondary" size="sm" onClick={() => setLoginHistoryInView(true)}>
                ...
                {events && events.results.length === 0 ? (
                  <p className="text-sm text-muted-foreground">{t('userDetailLoginHistoryEmpty')}</p>
                ) : null}
                {events && events.results.length > 0 ? (
                  <>
                  <div className={cn('relative rounded-md border border-border', loadingEvents && 'opacity-60')}>
                    {loadingEvents ? (
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="bg-muted/30 hover:bg-muted/30">
                            ...
                            <TableHead className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailLoginColProvider')}</TableHead>
                            <TableHead className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t('userDetailLoginColOutcome')}</TableHead>
                        ...
                        <TableBody className="font-mono text-xs">
                          {events.results.map((ev) => (
                            <TableRow key={ev.id}>
                              ...
                              <TableCell>
                                <span
                                  className={cn(
                                    ...
                                    ev.outcome === 'success' ? 'bg-emerald-500/15 text-emerald-800 dark:text-emerald-200' : 'bg-destructive/15 text-destructive',
                                  )}
                                >
                                  {ev.outcome}
                                </span>
                                {ev.failure_reason ? (
                                  <span className="mt-1 block max-w-[14rem] truncate text-[10px] text-muted-foreground" title={ev.failure_reason}>
                                    {ev.failure_reason}
                                  </span>
                                ) : null}
                              </TableCell>
                              ...
                              <TableCell className="hidden lg:table-cell">{ev.client_country || '—'}</TableCell>
                              <TableCell className="hidden xl:table-cell max-w-[20rem] truncate text-[10px]" title={ev.user_agent}>
                                {ev.user_agent || '—'}
                              </TableCell>
                            </TableRow>
                          ))}
